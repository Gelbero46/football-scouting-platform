...
export default function EditPlayerPage() {
  ...
  const fetchPlayer = async () => {
    try {
      ...
      const response = await api.players.get(params.id)
      ...
      // ✅ Normalize undefined/null values to empty string for text inputs
      const safePlayer = Object.fromEntries(
        Object.entries(player).map(([k, v]) => [k, v ?? ""])
      )
      console.log("safePlayer", safePlayer)

      
      // ✅ Reset after ensuring types match the form
      form.reset(safePlayer as PlayerFormValues)
    } catch (err) {
  ...
  const onSubmit = async (data: PlayerFormValues) => {
    ...
    try {
      ...
      await api.players.update(params.id, cleanData)
      ...
      setError(err.response?.data?.detail || 'Failed to edit player')
